...
#Soronzonbold Otgonbaatar: this code is based on tensornetwork library (see https://github.com/google/TensorNetwork/tree/master)
...
class MPS(Layer):
  ...
  """Matrix Product State (MPS) TN layer.
  ...
  Example:
    ::
      ...
      model = Sequential()
      model.add(
        MPS(1024, num_nodes=4, bond_dim=8, activation='relu',
        input_shape=(1024,)))
      # now the model will take as input arrays of shape (*, 1024)
      # and output arrays of shape (*, 1024).
      # After the first layer, you don't need to specify
      # the size of the input anymore:
      model.add(MPS(1024, num_nodes=4, bond_dim=8, activation='relu'))
  ...
  def __init__(self,
    ...
    assert num_nodes > 2, 'Need at least 3 nodes to create MPS.'
...
#      {self.output_dim}**(1. / {self.num_nodes}) must be round.'
    ...
    self.in_leg_dim = 2 #math.ceil(input_dim**(1. / self.num_nodes))
    self.out_leg_dim = self.output_dim #math.ceil(self.output_dim**(1. / self.num_nodes))
    ...
    for i in range(self.num_nodes - 2):
        if(i==(self.num_nodes-2)/2):
            self.nodes.append(self.add_weight(name=f'middle_node_{i}', shape=(self.in_leg_dim, self.bond_dim,  self.bond_dim, self.out_leg_dim),
            trainable=True, initializer=self.kernel_initializer))
        else:
            self.nodes.append(self.add_weight(name=f'middle_node_{i}', shape=(self.in_leg_dim, self.bond_dim, self.bond_dim),
            trainable=True, initializer=self.kernel_initializer))
                              
    self.nodes.append(self.add_weight(name='end_node_last', shape=(self.in_leg_dim, self.bond_dim), trainable=True, initializer=self.kernel_initializer))
  ...
  def call(self, inputs: tf.Tensor, **kwargs) -> tf.Tensor:  # pylint: disable=unused-argument, arguments-differ
    ...
    def feature_map(self, x: tf.Tensor)-> tf.Tensor:
        return tf.reshape([tf.math.cos((math.pi/2)*x), tf.math.sin((math.pi/2)*x)], (2, x.shape[-1]))
...
#      l = [in_leg_dim] * num_nodes
      input_reshaped = feature_map(self, x) #tf.reshape(x, tuple(l))
#      x_node = tn.Node(input_reshaped, name='xnode', backend="tensorflow")
      
      x_nodes = []

      for i in range(input_reshaped.shape[-1]):
        x_nodes.append(tn.Node(input_reshaped[:,i], name=f'x_node_{i}', backend="tensorflow"))
       
      tn_nodes = []
      for i, v in enumerate(nodes):
        tn_nodes.append(tn.Node(v, name=f'node_{i}', backend="tensorflow"))
        # Connect every node to input node
        x_nodes[i][0] ^ tn_nodes[i][0]
...
#      tn_nodes[0][1] ^ tn_nodes[1][1]
      for i, _ in enumerate(tn_nodes):
        ...
        if i!=0 and i!=len(tn_nodes)-1:
            tn_nodes[i][2] ^ tn_nodes[i + 1][1]
        if i == len(tn_nodes)-1:
            break

      # The MPS should now look like this
      #         |
      #   1 --- 2 --- 3 ---...--- 784
      #   |     |     |            |
      #   x[1]  x[2]  x[3]        x[784]
      ...
      temp = x_nodes[0] @ tn_nodes[0]
      for i in range(1, len(tn_nodes)):
        temp = x_nodes[i] @ tn_nodes[i] @ temp
